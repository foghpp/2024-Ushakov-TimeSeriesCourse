...
class UCR_DTW(BestMatchFinder):
    ...
    def _LB_Keogh(self, subs1: np.ndarray, subs2: np.ndarray, r: float) -> float:
        ...
        for i in range(len(subs1)):
            ...
            if subs2[i] > u:
                lb_Keogh += (subs2[i] - u)**2
            elif subs2[i] < l:
                lb_Keogh += (subs2[i] - l)**2
    ...
    def perform(self, ts_data: np.ndarray, query: np.ndarray) -> dict:
        ...
        for i in range(N - m + 1):
            ...
            LB_KimFL = self._LB_Kim(query, ts_data[i])
            if LB_KimFL < bsf:
                self.lb_Kim_num += 1
                LB_KeoghEQ = self._LB_Keogh(query, ts_data[i], self.r)
                if LB_KeoghEQ < bsf:
                    self.lb_KeoghQC_num += 1
                    LB_KeoghEC = self._LB_Keogh(ts_data[i], query, self.r)
                    if LB_KeoghEC < bsf:
