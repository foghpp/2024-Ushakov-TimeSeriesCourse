...
def plot_bestmatch_results(ts: np.ndarray, query: np.ndarray, bestmatch_results: dict) -> None:
    ...
    # Visualizing the best match results
    for i in bestmatch_results['indices']:
        fig.add_trace(go.Scatter(x=np.arange(query_len) + i, y=ts[i : i+query_len], line=dict(color=px.colors.qualitative.Plotly[1])),
                row=1, col=2)
